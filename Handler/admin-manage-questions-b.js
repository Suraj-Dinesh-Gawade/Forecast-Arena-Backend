...
export const manageQuestion = (req, res) => {
  const sql =
    "SELECT q_id, question, Category, Odd_One, Odd_Two, status  FROM Questions";
  db.query(sql, (err, result) => {
    if (err) {
      return res.status(500).json({
        Error:
          "Database Error! Error occured while fetching the question from the ForecastArena",
      });
    }
...
export const resolveQuestion = (req, res) => {
  ...
  db.query(sql, [questionId], (err) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ Error: "Database Error! Error while Resolving Question" });
    }
...
export const deleteQuestion = (req, res) => {
  ...
  db.query(sql, [questionId], (err) => {
    if (err) {
      return res
        .status(500)
        .json({ Error: "Database Error! Failed to Delete Question" });
    }
...
export const winOption = (req, res) => {
  ...
  if (!qId || !winnervalue) {
    return res
      .status(400)
      .json({ Error: "Question ID and winner value are required!" });
  }

  const selectSql =
    "SELECT Odd_One, Odd_Two, Yes_Odds, No_Odds  FROM Questions WHERE q_id = ?";
  ...
  db.query(selectSql, [qId], (fetchErr, qRows) => {
    if (fetchErr) {
      ...
      return res
        ...
        .json({
          Error:
            "Failed to retrieve options details for prediction calculation.",
        });
    }
    ...
    const winningOdds = isOptionOneWinner
      ? Number(question.Yes_Odds || 1.8)
      : Number(question.No_Odds || 1.8);

    const sql =
      'UPDATE Questions SET winner = ?, status = "Settled" WHERE q_id = ?';
    ...
    db.query(sql, [winnervalue, qId], (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({
            Error: "Database Error! Failed to store winner into ForecastArena",
          });
      }
      ...
      db.query(updateBetsSql, [winnervalue, qId], (betsErr) => {
        if (betsErr) {
          ...
          return res
            .status(500)
            .json({
              Error: "Question settled, but failed to update win/lose statuses",
            });
        }

        const getBetsSql =
          "SELECT bet_id, b_user_id, selected_option, bet_amount FROM Bets WHERE b_question_id = ?";
        ...
        db.query(getBetsSql, [qId], (getBetsErr, betsList) => {
          if (getBetsErr) {
            ...
            return res
              ...
              .json({
                Error:
                  "Failed to retrieve user prediction history for calculation.",
              });
          }
          ...
          let hasErrors = false;
          const hasWinner = betsList.some(
            (bet) =>
              bet.selected_option.toLowerCase() === winnervalue.toLowerCase(),
          );
          betsList.forEach((bet) => {
            ...
            if (isWin) {
              ...
              if (!hasWinner) {
                // Nobody predicted the winner -> system earns all losing bets

                const logLossSql =
                  "INSERT INTO System_Profit_Audit (user_id, q_id, amount_lost) VALUES (?, ?, ?)";
                  
                db.query(logLossSql, [userId, qId, betAmount], (auditErr) => {
                  if (auditErr)
                    console.error("Failed to log audit record:", auditErr);
                });
                ...
                // Winners exist -> don't transfer anything to system
                updateBalanceSql = null;
              }
            }

            // Execute updates
            const updateBetCoinsSql =
              "UPDATE Bets SET Coins_Won_Lose = ? WHERE bet_id = ?";
            db.query(
              updateBetCoinsSql,
              [coinsChange, betId],
              (betUpdateErr) => {
                if (betUpdateErr) {
                  console.error(
                    `Failed to update bet coins for ID ${betId}:`,
                    betUpdateErr,
                  );
                  hasErrors = true;
                  next();
                  ...
                  if (updateBalanceSql) {
                    db.query(
                      ...
                      (userUpdateErr) => {
                        if (userUpdateErr) {
                          console.error(
                            `Failed to adjust balance for ID ${userId}:`,
                            userUpdateErr,
                          );
                          hasErrors = true;
                        } else if (isWin) {
                          const logTxSql =
                            "INSERT INTO Transactions (t_user_id, amount, type) VALUES (?, ?, 'Prediction Win Payout')";
                          db.query(logTxSql, [userId, coinsChange], (txErr) => {
                            if (txErr)
                              console.error(
                                "Payout transaction log failed:",
                                txErr,
                              );
                          });
                        }
              ...
              if (completedQueries === betsList.length) {
                if (hasErrors) {
                  return res
                    .status(200)
                    .json({
                      message: `Settlement complete, but some transfers failed.`,
                    });
                }
                res
                  .status(200)
                  .json({
                    message: `Winner [${winnervalue}] registered! Payouts and System Reserved transfers updated.`,
                  });
              }
            }
          });
